...
void calculate() {
    ...
    float mBB, dRBB, pTV, dPhiVBB,bin_MV2c10B2,MET,bin_MV2c10B1,pTB2,pTB1,MEff,dEtaBB,sumPtJets; // 变量必须是 float 类型，因为 TMVA Reader 只能处理 float
    // 必须把变量地址绑定给 Reader
    ...
    for(auto r : readers) {
        ...
        r->AddVariable("MET", &MET);
        r->AddVariable("bin_MV2c10B1", &bin_MV2c10B1);
        r->AddVariable("pTB2", &pTB2);
        r->AddVariable("pTB1", &pTB1);
        r->AddVariable("MEff", &MEff);
        r->AddVariable("sumPtJets", &sumPtJets);
        r->AddVariable("dEtaBB", &dEtaBB);
    ...
    readerEven->BookMVA("BDT_Even", "/home/haoran/tmva/tmva_12vars/dataset_Even/weights/TMVAClassification_Even_BDT_nJ2.weights.xml");
    readerOdd->BookMVA("BDT_Odd", "/home/haoran/tmva/tmva_12vars/dataset_Odd/weights/TMVAClassification_Odd_BDT_nJ2.weights.xml");
    ...
    // 3. 绑定 Tree 变量 (注意类型匹配)
    // ---------------------------------------------------------
    float tree_mBB, tree_dRBB, tree_pTV, tree_dPhiVBB, tree_bin_MV2c10B2,tree_MET,tree_bin_MV2c10B1,tree_pTB2,tree_pTB1,tree_MEff,tree_dEtaBB,tree_sumPtJets;
    float eventWeight;
    int nJ;
    ...
    theTree->SetBranchAddress("MET", &tree_MET);
    theTree->SetBranchAddress("bin_MV2c10B1", &tree_bin_MV2c10B1);
    theTree->SetBranchAddress("pTB2", &tree_pTB2);
    theTree->SetBranchAddress("pTB1", &tree_pTB1);
    theTree->SetBranchAddress("MEff", &tree_MEff);
    theTree->SetBranchAddress("dEtaBB", &tree_dEtaBB);
    theTree->SetBranchAddress("sumPtJets", &tree_sumPtJets);
