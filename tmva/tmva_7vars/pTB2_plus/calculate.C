...
void calculate() {
    ...
    float mBB, dRBB, pTV, dPhiVBB,bin_MV2c10B2,MET,pTB2; // 变量必须是 float 类型，因为 TMVA Reader 只能处理 float
    // 必须把变量地址绑定给 Reader
    ...
    readerEven->BookMVA("BDT_Even", "/home/haoran/tmva/tmva_7vars/pTB2_plus/dataset_Even/weights/TMVAClassification_Even_BDT_nJ2.weights.xml");
    readerOdd->BookMVA("BDT_Odd", "/home/haoran/tmva/tmva_7vars/pTB2_plus/dataset_Odd/weights/TMVAClassification_Odd_BDT_nJ2.weights.xml");
    ...
    // 3. 绑定 Tree 变量 (注意类型匹配)
    // ---------------------------------------------------------
    float tree_mBB, tree_dRBB, tree_pTV, tree_dPhiVBB, tree_bin_MV2c10B2,tree_MET,tree_pTB2;
    float eventWeight;
    int nJ;
